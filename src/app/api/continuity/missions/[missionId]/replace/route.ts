...
export async function POST(request:Request,{params}:{params:Promise<{missionId:string}>}){try{const body=schema.parse(await request.json());const {missionId}=await params;return Response.json(await continuityService.replace(missionId,body.needId,body.expectedVersion));}catch(error){return continuityErrorResponse(error);}}
